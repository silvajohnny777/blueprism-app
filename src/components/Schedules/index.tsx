...
const Schedules: React.FC = () => {
  ...
  const isLoading = useSelector(
    (state: RootState) => state.schedules.isLoadingSchedules
  );
  const hadAnAPIError = useSelector(
    (state: RootState) => state.schedules.scheduleError
  );
  const scheduleSearch = useSelector(
    (state: RootState) => state.schedules.searchValue
  );
  ...
  return (
    <div
      className={`md:h-[calc(100vh-100px)] mb-[15px] md:mb-[0px] overflow-auto sticky top-[100px] p-[10px] pl-0`}
    >
      ...
      <div className="block flex overflow-auto md:flex-col">
        {isLoading ? (
          ...
          <p className="break-words">
            Schedule <span className="font-bold">{scheduleSearch}</span> not
            found!
          </p>
        ) : (
          "No schedules"
