...
const ScheduleItem: React.FC<ScheduleItemTypes> = ({ schedule }) => {
  ...
  useEffect(() => {
    const handleKeyPress = (event: KeyboardEvent) => {
      if (event.key === "Enter") {
        const key = {} as React.MouseEvent<HTMLElement>;
        handleClick(key, id);
      }
    };
  ...
  const handleClickRetire = async (
    ...
    setLoading(false);
    const position = schedulesArrayHandler.findIndex(
      (schedule: ScheduleTypes) => schedule.id === id
    );
  ...
  return (
    <div
      ...
      onKeyDown={(e: React.KeyboardEvent<HTMLElement>) =>
        e.key === "Enter" ? handleClick(e, id) : null
      } // Pass focusedIndex to handleKeyDown
      className={`relative p-[5px] ${
        filteredScheduleID === id ? "border-[#0f7dc2]" : ""
      } w-full rounded-[10px] mb-[15px] border-solid border-2 flex mr-[5px] md:mr-0 cursor-pointer hover:bg-[#ebf3f3] transition ease duration-200 ${
        filteredScheduleID === id ? "bg-[#ebf3f3]" : ""
      }`}
      tabIndex={0}
    >
        ...
        {isRetired && (
          <p
            className={`italic text-[12px] ${
              filteredScheduleID !== id ? "text-slate-400" : "font-bold"
            }`}
          >
            {filteredScheduleID !== id
              ? "No logs are being shown for this schedule"
              : "Showing achieved logs"}
          </p>
        )}
      </div>
      <button
        className={`z-10 bg-[#ffffff] button absolute bottom-[5px] right-[5px] border-2 border-solid px-[5px] py-[2px] w-[80px] rounded-full ${
          loading ? "pointer-events-none" : ""
        }`}
        onClick={(e: React.MouseEvent<HTMLElement>) => handleClickRetire(e, id)}
      >
        {loading ? "Loading..." : isRetired ? "Unretire" : "Retire"}
      </button>
    </div>
  );
};
