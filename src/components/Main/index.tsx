...
const Main: React.FC = () => {
  ...
  const logSearch = useSelector(
    (state: RootState) => state.scheduleLogs.searchLogValue
  );

  const schedulesLogs = useSelector(
    (state: RootState) => state.scheduleLogs.data
  );
  const isLoading = useSelector(
    (state: RootState) => state.scheduleLogs.isLoadingLogsSchedules
  );
  const hadAnAPIError = useSelector(
    (state: RootState) => state.scheduleLogs.scheduleLogsError
  );
  const schedules = useSelector((state: RootState) => state.schedules.data);
  const filteredScheduleID = useSelector(
    (state: RootState) => state.schedules.filteredScheduleID
  );

  const retiredIDs = schedules.filter(
    (schedule: ScheduleTypes) => schedule.isRetired
  );

  const filteredSchedulesLogs = filteredScheduleID
    ? schedulesLogs.filter(
        (log: ScheduleLogTypes) => log.scheduleId === filteredScheduleID
      )
    : schedulesLogs.filter((log: ScheduleLogTypes) => {
        const scheduleMatchID = retiredIDs.findIndex(
          (retired: ScheduleTypes) => retired.id === log.scheduleId
        );
  ...
  return (
    <div className="h-[calc(100vh-100px)] overflow-auto p-[10px] w-[100%]">
      <h2 className="text-[20px] font-bold mb-[25px]">
        {isLoading
          ? "Getting logs..."
          : filteredSchedulesLogs.length > 0
          ? `${filteredSchedulesLogs.length} logs being shown ${
              filteredScheduleID ? `for schedule ${filteredScheduleID}` : ""
            }`
          : filteredScheduleID
          ? ``
          : `No logs were found!`}
        ...
        <div className="relative w-[300px]">
          ...
          <input
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
              dispatch(setSearchLogValue(e.target.value))
            }
            value={logSearch}
          />
        </div>
          ...
          filteredSchedulesLogs.map((log: ScheduleLogTypes) => {
            ...
            return (
              <ScheduleLogItem
                key={log.id}
                log={log}
                isRetired={retiredIDs[scheduleMatchID]?.isRetired}
              />
            );
          })
        )}
